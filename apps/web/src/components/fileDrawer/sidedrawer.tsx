...
import { groupFilesByOrigin } from "../../utils/api/file.api";
import { createSignal, Show, Suspense, createEffect } from "solid-js";
import { hoveredItem } from "../../stores/inventory-store";
...
function HighlightedCode(props: { content: any; lang: string }) {
  const html = () => {
    ...
    if (typeof props.content === 'string') {
      ...
      // TypeScript now allows this because 'content' isn't strictly a string anymore
...
function MarkdownPreview(props: { content: string }) {
  ...
  return (
    <div class="rounded-lg border border-base-300 bg-base-300/40 overflow-hidden h-full flex flex-col">
      ...
      <div
        class="overflow-auto flex-1 p-4 prose prose-sm prose-invert max-w-none
               prose-pre:bg-base-300/60 prose-pre:border prose-pre:border-base-300
               prose-code:text-primary/80 prose-headings:text-base-content/90"
        innerHTML={html()}
      />
    </div>
  );
...
export default function SideDrawer() {
  ...
  const content = createAsync(() => getItemContent(hoveredItem()?.id || '', runId()));
  ...
  // Resolve the full file object (with name) from the already-loaded invoices list
  const hoveredFile = () => {
    const item = hoveredItem();
    if (!item) return null;
    const all = [...(invoices()?.workspace ?? []), ...(invoices()?.artifact ?? [])];
    return all.find(f => f.id === item.id) ?? null;
  };

  const fileExt = () => {
    const name = hoveredFile()?.name ?? '';
    return name.includes('.') ? name.split('.').pop()! : 'plaintext';
  };

  const isMarkdown = () => ['md', 'mdx'].includes(fileExt());

  // Reset panel state when focused file changes
  createEffect(() => {
    hoveredItem();
    setIsEditing(false);
    ...
    <div class="drawer drawer-end fixed inset-0 pointer-events-none z-40">
      ...
      <div class="drawer-side z-40">
        ...
        <div class="flex flex-col h-full w-2xl bg-base-200 shadow-xl">
          ...
          {/* ── File grid ── */}
          <div class="flex-2 min-h-0 overflow-y-auto p-3 border-b border-base-300">
            <p class="text-[10px] font-mono text-primary/50 tracking-widest mb-2 px-1">
              {activeMenu().toUpperCase()}://
            </p>
            <Suspense fallback={
              <div class="grid grid-cols-3 gap-2">
                {Array.from({ length: 9 }).map(() => (
                  <div class="aspect-square rounded-lg bg-base-300/60 animate-pulse" />
                ))}
              </div>
            }>
              <Show when={activeMenu() === 'workspace'}>
                <Inventory type="file" items={invoices()?.workspace || []} renderItem={(file) => <FileIcon file={file} showName={true} />} />
              </Show>
              <Show when={activeMenu() === 'artifact'}>
                <Inventory type="file" items={invoices()?.artifact || []} renderItem={(file) => <FileIcon file={file} showName={true} />} />
              </Show>
            </Suspense>
          </div>

          {/* ── Preview panel ── */}
          <div class="flex-3 flex flex-col min-h-0 p-3">
            ...
            <div class="flex items-center justify-between mb-2 px-0.5 shrink-0">
              ...
              <Show when={content()}>
                <div class="flex items-center gap-1.5">
                  ...
                  {/* EDIT / SAVE / CANCEL — non-images only */}
                  ...
                  {/* DELETE — always visible, hidden while editing */}
                  <Show when={!isEditing()}>
                    <Show
                      when={confirmDelete()}
                      fallback={
                        <ActionBtn label="DELETE" variant="danger" onClick={() => setConfirmDelete(true)} />
                      }
                    >
                ...
                <Show
                  ...
                  fallback={
                    <div class="flex flex-col items-center justify-center h-full text-base-content/25 gap-2 rounded-lg border border-dashed border-base-300">
                      ...
                      <span class="text-xs">Hover a file to preview</span>
                    </div>
                  }
                  keyed
                >
